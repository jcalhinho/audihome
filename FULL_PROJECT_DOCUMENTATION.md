...
## 2. 📊 SCHÉMAS D'INTERACTION

### A. Flux Audio (Web Audio API Graph)
Ce schéma montre comment le son circule physiquement dans le navigateur.

```mermaid
graph LR
    URL[Flux Radio/Micro] -->|Fetch| AE[HTMLAudioElement]
    AE -->|Source| SN[SourceNode]
    SN -->|Gain| GN[GainNode (Volume/Atténuation)]
    GN -->|Analyse| AN[AnalyserNode (FFT)]
    AN -->|Sortie| AD[AudioDestination (Enceintes)]
    AN -.->|Données Fréquences| CV[Visualiseur Canvas]
    AN -.->|Calcul dB| EC[Graphique ECharts]
```

### B. Flux de Données & États (Svelte)
Ce schéma montre comment les données (Props) et les ordres (Callbacks) circulent entre les composants.

```mermaid
graph TD
    %% État Global
    Root[+page.svelte (ÉTAT GLOBAL)]

    %% Descente de données (Props)
    Root -->|volume, current, isPlaying| PC[PlayerCard]
    Root -->|zones, labels| ZC[ZonesCard]
    Root -->|attenuation, zoneVolume| CC[ControlsCard]
    Root -->|simSpeed| CH[ChartCard]

    %% Références (Bindings)
    PC -.->|bind:canvasEl| Root
    CH -.->|bind:chartEl| Root

    %% Remontée d'événements (Callbacks)
    PC -->|play/pause/scroll| Root
    ZC -->|toggleZone| Root
    CC -->|onZoneVolumeInput| Root
    CC -->|selectAttenuation| Root
```
...
## 4. 🧠 LE CERVEAU : `src/routes/+page.svelte`
...
## 5. 🧪 LA COUCHE LOGIQUE (`src/lib/logic/`)
...
## 6. 🧱 ANATOMIE DES COMPOSANTS (`src/lib/components/`)
...
## 9. 💡 RÉPONSES AUX "POURQUOI ?" (Soutenance)
